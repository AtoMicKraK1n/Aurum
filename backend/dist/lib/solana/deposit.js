...
exports.verifyUsdcDeposit = verifyUsdcDeposit;
...
async function verifyUsdcDeposit(params) {
    ...
    const senderInAccounts = tx.transaction.message.accountKeys.some((key) => key.pubkey.toBase58() === params.senderWallet);
    if (!senderInAccounts) {
    ...
    if (received + 1e-9 < params.expectedUsdcAmount) {
        throw new Error(`Insufficient treasury USDC received. Expected ${params.expectedUsdcAmount}, got ${received}`);
    }
